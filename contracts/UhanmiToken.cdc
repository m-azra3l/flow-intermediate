...
// UhanmiToken contract: Implements fungible token on Flow blockchain
pub contract UhanmiToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @UhanmiToken.Vault
        ...
        // Destroy the vault
        destroy() {
            UhanmiToken.totalSupply = UhanmiToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            UhanmiToken.totalSupply = UhanmiToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&UhanmiToken.Minter>(/public/Minter, target: /storage/MinterStorage)
