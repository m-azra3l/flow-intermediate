...
// SwapToken contract: Facilitates token swapping between UhanmiToken and FlowToken
pub contract SwapToken {
    ...
    // Function to swap tokens between UhanmiToken and FlowToken
    pub fun swapTokens(signer: AuthAccount, swapAmount: UFix64) {

        // Borrow UhanmiToken and FlowToken vaults from the signer's storage
        let uhanmiTokenVault = signer.borrow<&UhanmiToken.Vault>(from: /storage/VaultStorage)
            ?? panic("Could not borrow UhanmiToken Vault from signer")

        let flowVault = signer.borrow<&FlowToken.Vault>(from: /storage/FlowVault)
            ?? panic("Could not borrow FlowToken Vault from signer")  

        // Borrow Minter capability from UhanmiToken
        let minterRef = signer.getCapability<&UhanmiToken.Minter>(/public/Minter).borrow()
        ...
        let newUhanmiTokenVault <- minterRef.mintToken(amount: mintedAmount)
        uhanmiTokenVault.deposit(from: <-newUhanmiTokenVault)
